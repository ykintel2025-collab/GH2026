...
def get_connection():
    ...
    Als de pool zelf vol zit (bijv. door een eerdere fout die een verbinding niet netjes
    teruggaf), valt dit terug op een rechtstreekse verbinding i.p.v. de app te laten vastlopen."""
    _t0 = time.time()
    pool = _get_pool()
    for _ in range(2):
        try:
            ...
            # Pool is vol — niet laten vastlopen, gewoon een losse verbinding erbij maken
...
def init_db():
    ...
    cur.execute("ALTER TABLE debts ADD COLUMN IF NOT EXISTS has_collector BOOLEAN DEFAULT FALSE")
    cur.execute("ALTER TABLE debts ADD COLUMN IF NOT EXISTS collector_name TEXT")
    cur.execute("ALTER TABLE debts ADD COLUMN IF NOT EXISTS collector_case_number TEXT")
    cur.execute("ALTER TABLE debts ADD COLUMN IF NOT EXISTS collector_contact_person TEXT")
    cur.execute("ALTER TABLE debts ADD COLUMN IF NOT EXISTS collector_notes TEXT")
    cur.execute("ALTER TABLE debts ADD COLUMN IF NOT EXISTS collector_phone TEXT")
    cur.execute("ALTER TABLE debts ADD COLUMN IF NOT EXISTS collector_email TEXT")
...
def update_debt(debt_id, **fields):
    ...
    allowed = {
        ...
        "has_collector", "collector_name", "collector_case_number", "collector_contact_person", "collector_notes",
        "collector_phone", "collector_email",
    }
...
def get_debts(status=None):
    ...
    base_query = """
        SELECT d.id, d.contact_id, d.total_amount, d.current_amount, d.priority, d.status, d.last_contact,
               ...
               d.has_collector, d.collector_name, d.collector_case_number, d.collector_contact_person, d.collector_notes,
               d.collector_phone, d.collector_email,
