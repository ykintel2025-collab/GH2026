            ...
            with st.expander(label):
                ...
                with tab_msg:
                    ...
                    with mcol1:
                        ...
                        wa_choice = st.selectbox("Sjabloon", list(wa_options.keys()), key=f"cwatpl_{c['id']}")
                        wa_default = fill_template(wa_options[wa_choice]["body"], c) if wa_options[wa_choice] else ""
                        wa_text = st.text_area("Bericht", value=wa_default, key=f"cwatext_{c['id']}")
                        wa_url = whatsapp_link(c.get("phone"), wa_text)
                        if wa_url:
                            st.link_button("📱 Open in WhatsApp", wa_url)
                        else:
                            st.caption("Geen telefoonnummer bekend — vul dit in bij de gegevens hierboven.")

                    with mcol2:
                        st.markdown("**✉️ E-mail**")
                        mail_options = {"— vrije tekst —": None}
                        mail_options.update({t["name"]: t for t in mail_templates})
                        mail_choice = st.selectbox("Sjabloon", list(mail_options.keys()), key=f"cmailtpl_{c['id']}")
                        mail_subject_default = fill_template(mail_options[mail_choice]["subject"], c) if mail_options[mail_choice] else ""
                        mail_body_default = fill_template(mail_options[mail_choice]["body"], c) if mail_options[mail_choice] else ""
                        mail_subject = st.text_input("Onderwerp", value=mail_subject_default or "", key=f"cmailsub_{c['id']}")
                        mail_body = st.text_area("Bericht", value=mail_body_default, key=f"cmailbody_{c['id']}")
                        if not c.get("email"):
                            st.caption("Geen e-mailadres bekend — vul dit in bij de gegevens hierboven.")
                        else:
                            if st.button("✉️ Verstuur e-mail", key=f"csendmail_{c['id']}"):
                                success, error = emailer.send_email(c["email"], mail_subject, mail_body)
                                if success:
                                    db.add_contact_log(c["id"], f"E-mail verzonden: '{mail_subject}'", logged_by=current_user)
                                    st.success(f"E-mail verzonden naar {c['email']}.")
                                    st.rerun()
                                else:
