...
def get_download_url(storage_path, expires_in=3600):
    ...
    Retourneert (url: str|None, foutmelding: str|None)."""
    try:
        ...
        if resp.status_code == 200:
            ...
            if signed_path:
                return st.secrets["SUPABASE_URL"].rstrip("/") + "/storage/v1" + signed_path, None
            return None, "Geen signedURL in het antwoord van Supabase."
...
def download_file(storage_path):
    ...
    Retourneert (bytes: bytes|None, foutmelding: str|None)."""
    try:
        url = f"{_base_url()}/object/{BUCKET}/{storage_path}"
